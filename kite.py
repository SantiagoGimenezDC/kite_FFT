...
class Calculation:
    ...
    @property
    def get_conductivity_dc_FFT(self):
        """Returns the requested DC conductivity functions."""
        return self._conductivity_dc_FFT

    @property
    def get_conductivity_dc_mod(self):
        """Returns the requested DC conductivity functions."""
        return self._conductivity_dc_mod
    ...
    def __init__(self, configuration=None):
        ...
        self._conductivity_dc_FFT = []
        self._conductivity_dc_mod = []
    ...
    def conductivity_dc_FFT(self, direction, num_points, num_moments, num_random, num_reps,  num_disorder=1, kernel=1, temperature=0):
        """Calculate the DC conductivity for a given direction, --using the FFT algorithm!--
        ...
        if direction not in self._avail_dir_full:
            ...
            self._conductivity_dc_FFT.append(
                {'direction': self._avail_dir_full[direction], 'num_points': num_points, 'num_moments': num_moments,
                 'num_random': num_random, 'num_disorder': num_disorder, 'num_reps': num_reps,
                 'temperature': temperature, 'kernel': kernel})


    def conductivity_dc_mod(self, direction, num_points, num_moments, num_random, num_reps,  num_disorder=1, temperature=0):
        """Calculate the DC conductivity for a given direction, --using the mod algorithm!--
        ...
        if direction not in self._avail_dir_full:
            ...
            self._conductivity_dc_mod.append(
                {'direction': self._avail_dir_full[direction], 'num_points': num_points, 'num_moments': num_moments,
                 'num_random': num_random, 'num_disorder': num_disorder, 'num_reps': num_reps,
                 'temperature': temperature})
...
def config_system(lattice, config, calculation, modification=None, **kwargs):
    ...
    **kwargs: Optional arguments like filename, Disorder or Disorder_structural.
    ...
    if calculation.get_conductivity_dc_FFT:
        grpc_p = grpc.create_group('conductivity_dc_FFT')

        moments, random, point, dis, temp, direction, numReps, kernel = [], [], [], [], [], [], [], []
        for single_cond_dc_FFT in calculation.get_conductivity_dc_FFT:
            moments.append(single_cond_dc_FFT['num_moments'])
            random.append(single_cond_dc_FFT['num_random'])
            point.append(single_cond_dc_FFT['num_points'])
            dis.append(single_cond_dc_FFT['num_disorder'])
            temp.append(single_cond_dc_FFT['temperature'])
            direction.append(single_cond_dc_FFT['direction'])
            numReps.append(single_cond_dc_FFT['num_reps'])
            kernel.append(single_cond_dc_FFT['kernel'])

        if len(calculation.get_conductivity_dc_FFT) > 1:
            raise SystemExit('Only a single function request of each type is currently allowed. Please use another '
                             'configuration file for the same functionality.')
        grpc_p.create_dataset('NumMoments', data=np.asarray(moments), dtype=np.int32)
        grpc_p.create_dataset('NumRandoms', data=np.asarray(random), dtype=np.int32)
        ...
        grpc_p.create_dataset('Temperature', data=np.asarray(temp) / config.energy_scale, dtype=np.float64)
        grpc_p.create_dataset('Direction', data=np.asarray(direction), dtype=np.int32)
        grpc_p.create_dataset('NumReps', data=np.asarray(numReps), dtype=np.int32)
        grpc_p.create_dataset('kernel', data=np.asarray(kernel), dtype=np.int32)


    if calculation.get_conductivity_dc_mod:
        grpc_p = grpc.create_group('conductivity_dc_mod')

        moments, random, point, dis, temp, direction, numReps = [], [], [], [], [], [], []
        for single_cond_dc_mod in calculation.get_conductivity_dc_mod:
            moments.append(single_cond_dc_mod['num_moments'])
            random.append(single_cond_dc_mod['num_random'])
            point.append(single_cond_dc_mod['num_points'])
            dis.append(single_cond_dc_mod['num_disorder'])
            temp.append(single_cond_dc_mod['temperature'])
            direction.append(single_cond_dc_mod['direction'])
            numReps.append(single_cond_dc_mod['num_reps'])

        if len(calculation.get_conductivity_dc_mod) > 1:
            raise SystemExit('Only a single function request of each type is currently allowed. Please use another '
                             'configuration file for the same functionality.')
        grpc_p.create_dataset('NumMoments', data=np.asarray(moments), dtype=np.int32)
        grpc_p.create_dataset('NumRandoms', data=np.asarray(random), dtype=np.int32)
